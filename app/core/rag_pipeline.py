import sys
import os

# Thêm thư mục gốc (chứa các file .py của bạn) vào sys.path để import
root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../'))
if root_dir not in sys.path:
    sys.path.append(root_dir)

import logging
from src.embedding import get_embedding_model
from src.retriever import retrieve_documents
from src.prompts import build_prompt
from src.generation import generate_answer

# Cấu hình logging cơ bản cho pipeline
logger = logging.getLogger("rag_pipeline")

def ask_question(query: str) -> str:
    """
    Luồng RAG chính: Query -> Embedding -> Retrieve -> Prompt -> LLM -> Answer
    """
    try:
        # Bước 1: Khởi tạo/Lấy model embedding và chuyển câu hỏi thành vector
        logger.info(f"Đang vector hóa câu hỏi: '{query}'")
        embed_model = get_embedding_model()
        
        # Mã hóa câu hỏi thành list các float
        query_embedding = embed_model.encode(query).tolist()

        # Bước 2: Tìm kiếm ngữ cảnh (top k tài liệu) trong Supabase
        logger.info("Đang truy xuất tài liệu từ cơ sở dữ liệu...")
        docs = retrieve_documents(query_embedding, top_k=5)
        
        if not docs:
            context = "Không tìm thấy tài liệu phù hợp trong cơ sở dữ liệu."
        else:
            # Gộp các chunk lại thành một đoạn ngữ cảnh duy nhất
            context = "\n\n---\n\n".join(docs)

        # Bước 3: Đưa ngữ cảnh và câu hỏi vào Prompt Template
        logger.info("Đang xây dựng prompt...")
        final_prompt = build_prompt(context=context, question=query)

        # Bước 4: Gọi LLM (Gemini) để tạo câu trả lời
        logger.info("Đang gọi Gemini...")
        answer = generate_answer(final_prompt)

        return answer

    except Exception as e:
        logger.error(f"Lỗi trong quá trình RAG: {e}")
        return f"Xin lỗi, đã xảy ra lỗi trong hệ thống: {str(e)}"